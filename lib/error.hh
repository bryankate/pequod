...
class ErrorHandler { public:
    ...
    /** @brief Return the count of errors reported via this handler.
     *
     ...
     * assert(errh1.count() == 0);
     * errh2.error("blah");
     * assert(errh1.count() == 1);
     * @endcode
     *
     * @sa account, clear */
     ...
     * updates the count() counter.  Some other ErrorHandlers
     ...
     *
     * The default implementation sets the count() counter to zero. */
    virtual void clear() {
	_nerrors = 0;
    }
    ...
    static bool has_converter(const String& name);
    static int add_converter(const String& name, Converter* converter);
    static Converter* remove_converter(const String& name);
...
class ErrorAccumulator : public ErrorHandler {
  public:
    ErrorAccumulator();

    inline void set_default_flags(int default_flags);
    String vformat(const char* fmt, va_list val);
    void* emit(const String& str, void* user_data, bool more);
    ...
    String join(const String& separator = String());
...
inline void ErrorAccumulator::set_default_flags(int default_flags) {
    default_flags_ = default_flags;
}

inline bool ErrorAccumulator::empty() const {
    return errors_.empty();
}

inline const String* ErrorAccumulator::begin() const {
    return errors_.data();
}

inline const String* ErrorAccumulator::end() const {
    return errors_.data() + errors_.size();
}
