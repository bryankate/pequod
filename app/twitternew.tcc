...
TwitterUser::TwitterUser(uint32_t uid)
    : TwitterGraphNode(uid), last_read_(0), last_subscribe_(0) {
}
...
TwitterNewPopulator::TwitterNewPopulator(const Json& param)
    : nusers_(param["nusers"].as_i(5000)),
      ...
      outpath_(param["outpath"].as_s("")),
      print_table_(param["print_table"].as_s("")),
...
void TwitterNewPopulator::make_users() {
    ...
    if (ngroups_ > 1)
        gend = nusers_ / ngroups_ + 1;

    group_uni_dist_ = uni_dist_type(0, gend - 1);
